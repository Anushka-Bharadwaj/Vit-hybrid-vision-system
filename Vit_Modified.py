...
from Hybrid_PSO_ACO_Feature_Visualization import hybrid_pso_aco_feature_selection
...
class Vision(nn.Module):
    def __init__(self, img_size=224, patch_size=16, emb_dim=768, depth=6, heads=8, mlp_dim=2048, num_classes=10, k=50, use_rope=False, use_conv_mlp=False, multi_scale=False, patch_sizes=[16, 8]):
        super().__init__()
        img_size = (img_size, img_size) if isinstance(img_size, int) else img_size
        if multi_scale:
            self.patch_embed = MultiScalePatchEmbedding(emb_dim, img_size, patch_sizes, 3)
            n_patches = sum(
                ((img_size[0] // 4) // ps) * ((img_size[1] // 4) // ps)
        ...
        self.pos_embed = PositionalEmbedding(n_patches + 1, emb_dim, use_rope=use_rope)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        self.feature_vector = x[:, 0]  # <-- Add this line to save the feature vector
...
def extract_blip_features(image_path):
    image = Image.open(image_path).convert("RGB")
    inputs = processor(images=image, return_tensors="pt").to(device)
    with torch.no_grad():
        outputs = blip_model.vision_model(**inputs)
        pooled = outputs.pooler_output.cpu().numpy().squeeze()
    return pooled

# Standard Vision Transformer (with RoPE and Conv-MLP options)
model = Vision(
    img_size=img_size[0],
    ...
    patch_sizes=[16, 8]            # <--- List your patch sizes here
)
...
lr = 0.005
epochs = 2
save_dir=r"C:\\Users\\Hp\\CascadeProjects\\vision_transformer\\Vit_Modified...A"
train_eval(model,lr,epochs,save_dir)

# --- Feature Extraction and Classification Pipeline ---

# 1. Extract features for all images in the test set (or any dataset)
model.eval()
features_list = []
labels_list = []
img_names = []
...
for idx, (inputs, labels) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        features = model.feature_vector.cpu().detach().numpy()
    features_list.append(features)
    labels_list.extend(labels.cpu().numpy())
    # Get image file names from dataset
    batch_start = idx * test_loader.batch_size
    batch_img_paths = [test_loader.dataset.samples[i][0] for i in range(batch_start, batch_start + len(labels))]
    img_names.extend(batch_img_paths)
    # BLIP captions for each image in batch
    for img_path in batch_img_paths:
        blip_caption = extract_caption(img_path)
...
# 2. Feature selection using hybrid PSO-ACO
selected_features = hybrid_pso_aco_feature_selection(X, y)
X_selected = X[:, selected_features == 1]

# 3. Train SVM classifier on selected features
clf = SVC()
clf.fit(X_selected, y)

# 4. Predict and print results for each image
for i, img_name in enumerate(img_names):
    x_feat = X[i, selected_features == 1].reshape(1, -1)
    predicted_class = clf.predict(x_feat)[0]
    blip_caption = blip_captions[i]
    print(f"Image: {img_name}")
    print(f"Predicted Class: {predicted_class}")
    print(f"BLIP Description: {blip_caption}")
    print("-" * 40)

# --- Feature Extraction and Classification Pipeline with Hybrid Features ---
...
for idx, (inputs, labels) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        outputs = model(inputs)
        features_vit = model.feature_vector.cpu().detach().numpy()
        
        # Extract BLIP features
        features_blip = []
        for img_path in img_names:
            blip_feature = extract_blip_features(img_path)
            features_blip.append(blip_feature)
        features_blip = np.array(features_blip)
        
    features_list_vit.append(features_vit)
    features_list_blip.append(features_blip)
    labels_list.extend(labels.cpu().numpy())
    img_names.extend([test_loader.dataset.samples[i][0] for i in range(idx * test_loader.batch_size, (idx + 1) * test_loader.batch_size)])

# Stack features and labels
X_vit = np.vstack(features_list_vit)
X_blip = np.vstack(features_list_blip)
y = np.array(labels_list)

# Concatenate ViT and BLIP features
X_hybrid = np.concatenate([X_vit, X_blip], axis=1)  # shape: (num_samples, vit_feature_dim + blip_feature_dim)

# 2. Feature selection using hybrid PSO-ACO
selected_features = hybrid_pso_aco_feature_selection(X_vit, X_blip, y)
X_selected = X_hybrid[:, selected_features == 1]
